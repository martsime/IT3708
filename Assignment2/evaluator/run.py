...
studentFolder = "my_out/216066" # you may have to specify the complete path
...
def readFilesFromFolder(directory):
        ...
        for filename in sorted(os.listdir(directory), key=natural_keys):
                if filename.endswith(".jpg") or filename.endswith(".png"):
                        ...
                        allFiles.append((filename,readImage(filename)))
                elif filename.endswith(".txt"):
                        filename = os.path.join(directory, filename)
                        allFiles.append((filename, readTextFile(filename)))
        return allFiles
...
def main():
        ...
        totalScore = 0
        for filename, student in studentFiles:
                highestScore = 0
                for _, opt in optimalFiles:
...
#                       print("PRI 2: %.2f" % result2)
                ...
                a = highestScore*100
                print(f"Score: {a:.2f}%, image: {filename}")
